...
public class AccelerometerGlassActivity extends Activity implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
        	...
        	int x_exp = (int) (1.5 * Math.abs(event.values[0]-center_x));
        	int y_exp = (int) (1.5 * Math.abs(event.values[2]-center_z));
        	
        	x -= (event.values[2]>0) ? x_exp : -x_exp;
        	if (x<=0)	x=0;
        	else if(x>=mMetrics.widthPixels-AnimatedView.width)	x=mMetrics.widthPixels-AnimatedView.width;
        	
            y += (event.values[2]>0) ? y_exp : -y_exp;
          	if (y<=0)	y=0;
